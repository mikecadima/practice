...
// DELETE a task
// app.get("/tasks", (req,res)=>res.sendFile(__dirname + path.join("/site/index.html")))

app.delete("/tasks/:id", (req, res) => {
    const taskId = req.params.id;
    let result = db.none("DELETE FROM tasks WHERE id = $1", [taskId]).then(() =>
      res.send(`Task ${taskId} was deleted`)
    );
  });
  app.patch('/edit-task/:id', async (req,res)=>{
    let del_result = await db.one(`
        DELETE FROM tasks
        WHERE id = '${req.params.id}' RETURNING *
    `);
    res.send(result);
